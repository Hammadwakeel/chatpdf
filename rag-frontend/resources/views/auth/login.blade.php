...
<div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8 bg-white p-10 rounded-3xl border border-gray-100 shadow-xl shadow-gray-200/50">
        <div class="text-center">
            ...
            <p class="mt-2 text-sm text-gray-500">Sign in to your ChatPDF account</p>
        </div>
        <form class="mt-8 space-y-6" action="{{ route('login.post') }}" method="POST">
            ...
            <div class="text-center mt-4">
                <a href="{{ route('signup') }}" class="text-sm font-medium text-indigo-600 hover:text-indigo-500">Don't have an account? Sign up</a>
            </div>
        </form>
    </div>
</div>
