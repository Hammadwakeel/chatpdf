...
def get_suggested_questions(
    ...
    """Retrieves the AI-generated suggested questions stored during document upload."""
    session = db.query(ChatSession).filter(
        ChatSession.session_id == request.session_id, 
        ChatSession.user_id == user.id
    ).first()
    ...
    # The rag.py script saves suggestions in the format "Suggested: Q1|Q2|Q3"
    if session.summary and session.summary.startswith("Suggested:"):
        raw_questions = session.summary.replace("Suggested: ", "").split("|")
